...
const s = $state({ session: 0, name: '', filter: '', avatarPath: null, avatarPreview: '', selected: 0, busy: false, status: '', error: false, profilesRev: 0 });

export function ccState() { return s; }
export function ccOpen() {
    s.session++; s.name = ''; s.filter = ''; s.avatarPath = null; s.avatarPreview = '';
...
export function ccProfilesChanged() { s.profilesRev++; }

...
{#if s.signer}
    <div class="remote-signer-card">
        <div class="remote-signer-head">
            <div class="remote-signer-title">
                <span class="remote-signer-dot {s.dot}" id="remote-signer-dot"></span>
                <span>{s.signer.label}</span>
            </div>
            <button class="cancel-btn" onclick={(e) => { e.preventDefault(); e.stopPropagation(); h.reauthorize(); }}>Re-authorize</button>
        </div>
        <div class="remote-signer-meta">
            <div class="remote-signer-meta-row">
                <span class="remote-signer-meta-label">Signer key</span>
                <span class="remote-signer-meta-value remote-signer-mono" title={s.signer.npub || ''}>{short}</span>
            </div>
        </div>
...
<div class="form-group">
    <label class="toggle-container">
        ...
        <input type="checkbox" bind:checked onchange={() => h.toggleEncryption(checked)}>
...
{#if s.enabled && (s.bioSupported || isBio)}
    <div class="danger-option">
        <div class="left-group">
            ...
            <span>{isBio ? 'Unlock: Biometrics' : `Unlock: ${credName}`}</span>
        </div>
        <button class="btn cancel-btn" onclick={() => h.switchUnlock()}>{isBio ? 'Use PIN' : 'Use Biometrics'}</button>
    </div>
{/if}
...
{#if s.enabled && !isBio}
    <div class="danger-option">
        <div class="left-group">
            ...
            <span>Change {credName}</span>
        </div>
        <button class="btn cancel-btn" onclick={() => h.changeCredential()}>Change</button>
    </div>
{/if}

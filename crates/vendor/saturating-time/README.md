# `saturating-time` (Vector-patched vendor of v0.4.0)

Vendored via `[patch.crates-io]` to fix an infinite loop on Windows: the
`find_limit` probe that discovers `SystemTime`'s platform MIN/MAX halves its
step down to 1ns, but Windows `SystemTime` counts 100ns intervals, so
sub-granularity steps round to zero and "succeed" unchanged — the probe never
terminates. `saturating_add`/`saturating_sub` also evaluated those limits
eagerly (`unwrap_or`), so the first saturating op on any Windows machine hung
forever. Both fixed here; drop the vendor once an upstream release ships the
fix (https://codeberg.org/cve/saturating-time).

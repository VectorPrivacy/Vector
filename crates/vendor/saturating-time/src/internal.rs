    ...
    F: Fn(&T, Duration) -> Option<T>,
{
    ...
    loop {
        ...
        let next = f(&res, step);
        match next {
            // A Some that didn't move is a silent no-op, not progress: on
            // platforms whose clock is coarser than 1ns (Windows SystemTime
            // counts 100ns intervals), sub-granularity steps round to zero and
            // "succeed" unchanged — treating that as progress loops forever.
            Some(st) if st != res => {
                // (2.1) If Some and the value advanced, keep it and continue.
                res = st
            }
            _ => {
                if step == ONE_NS {
                    // (2.2) At the smallest step, no progress means we are at
                    // the limit within this platform's clock granularity.
                    return res;
                } else {

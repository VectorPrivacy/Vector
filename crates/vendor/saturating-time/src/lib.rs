...
pub trait SaturatingTime: internal::SaturatingTime {
    ...
    fn saturating_add(self, duration: Duration) -> Self {
        // Lazily: max_value() runs a probe loop behind a LazyLock; an eager
        // unwrap_or would pay it on every call, even when checked_add succeeds.
    ...
    fn saturating_sub(self, duration: Duration) -> Self {
        // Lazily, for the same reason as saturating_add.

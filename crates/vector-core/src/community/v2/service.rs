...
pub async fn send_webxdc_signal<T: Transport + ?Sized>(
    ...
    publish_chat(transport, community, &group, author_pk, channel_id, epoch, rumor, at_ms, false).await.map(|_| ())
}
...
pub async fn fetch_channel_history<T: Transport + ?Sized>(
    ...
    crate::db::scoped(async move {
        ...
        for _ in 0..max_pages {
            ...
            // relays (Ditto) require the connection authed as the author queried and
            // reject a multi-author REQ ("all authors must be authenticated"), so a
...
/// Who an invite bundle is FOR — which decides the Private-Channel keys it may
/// carry (CORD-05 §1 vs §2).
///
/// A **Link** has no recipient: "anyone the link reaches can join", so its
/// audience holds no Role by construction and is entitled to no Private Channel
/// at all. A **Member** is a specific npub whose entitlement is computable.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum BundleAudience {
    /// A public link (33301 bundle event): public channels only.
    Link,
    /// A direct invite (3313) to this npub: may carry Private-Channel keys.
    Member(PublicKey),
}

/// Build the §1 invite bundle for this community, scoped to `audience`. A
/// Public channel carries the `community_root` as its "key" (the joiner derives
/// the real secret from the root); a Private one its own key — and only for a
/// Member the folded roster shows entitled. The bundle self-certifies the owner,
/// so the inviter's identity is irrelevant to trust.
pub fn bundle_of(
    community: &CommunityV2,
...
/// [`bundle_of`] settling entitlement against a Grant this client JUST published
/// (`with`/`without` role ids), since the fold lags its own publish. This is the
/// grant-vend path (CORD-03 "delivered on grant").
...
/// Transport failure is Err, NOT None: the 13303 is REPLACEABLE, so a caller
/// that mistakes "couldn't reach the relays" for "no list yet" and publishes a
/// fresh one wipes every link minted on other devices. Full evidence for the
/// same reason — this read feeds replaceable-event writes.
...
async fn publish_invite_registry<T: Transport + ?Sized>(transport: &T, community: &CommunityV2, __session: &std::sync::Arc<crate::db::Session>, live_signers: &[PublicKey]) -> Result<(), String> {
    ...
    publish_control_edition(transport, community, vsk::INVITE_LINKS, &eid, &content).await?;
...
async fn record_minted_link<T: Transport + ?Sized>(transport: &T, community: &CommunityV2, minted: &MintedLink) -> Result<(), String> {
    crate::db::scoped(async move {
        ...
        // a retry re-records it) — an unreachable relay set must never be mistaken
        // for "no list yet" and clobber the replaceable 13303. Ok(None) IS a fresh
        // creator's honest first list.
...
// ── Severing containment (CORD-06 Severing Refounding) ──────────────────────

/// Persisted sever marks for a community: `root_epoch -> rotation at_secs`.
/// A mark means "epoch E was a containment": link severance retries against it
/// on every follow pass, and a rescue extension of that hop re-stamps the tag.
pub(crate) fn sever_marks(cid_hex: &str) -> std::collections::BTreeMap<u64, u64> {
    crate::db::settings::get_sql_setting(format!("v2_sever:{cid_hex}"))
        .ok()
        .flatten()
...
pub(crate) fn add_sever_mark(cid_hex: &str, epoch: u64, at_secs: u64) {
    let mut marks = sever_marks(cid_hex);
    marks.insert(epoch, at_secs);
    if let Ok(v) = serde_json::to_string(&marks) {
        let _ = crate::db::settings::set_sql_setting(format!("v2_sever:{cid_hex}"), v);
    }
}

/// Sever MY OWN condemned public links: revoke (not refresh) every live link of
/// mine for this community whose minted-at epoch sits below the highest severed
/// epoch. The epoch is the discriminator — deterministic on every device, so a
/// second device adopting the containment late can never eat the fresh link the
/// owner minted to re-open (it carries the post-raid epoch), while a link minted
/// mid-raid by a not-yet-adopted device carries the raid-era epoch and dies.
/// Batched: every tombstone publishes, then ONE 13303 republish and ONE registry
/// edition for the whole set. Deliberately WITHOUT `revoke_public_link`'s
/// last-link refound escalation: every caller here sits inside a rotation
/// (refound or follow), where that escalation self-deadlocks on `follow_lock` —
/// and the severed rotation IS the read-cut the escalation exists to provide.
/// Returns the number revoked; `Ok(0)` when there is nothing of mine to sever.
async fn sever_own_links<T: Transport + ?Sized>(
    transport: &T,
    community: &CommunityV2,
) -> Result<usize, String> {
    crate::db::scoped(async move {
        ...
        let my_pk = me_pk()?;
        let cid_hex = crate::simd::hex::bytes_to_hex_32(&community.id().0);
        let Some(condemn_below) = sever_marks(&cid_hex).keys().copied().max() else {
            return Ok(0); // no containment on record — nothing is condemned.
        };
        // Local fast-path: the folded Registry names every creator with a live
        // link, and my own mints also leave local rows. Neither names me → no
        // 13303 fetch; a plain member pays nothing here.
        let maybe_mine = crate::db::community::invite_link_set_creators(&cid_hex)
            .map(|cs| cs.contains(&my_pk.to_hex()))
            .unwrap_or(true)
            || !crate::db::community::list_public_invites(&cid_hex).unwrap_or_default().is_empty();
        if !maybe_mine {
            return Ok(0);
        }
        let Some(mut list) = fetch_invite_list(transport, &community.relays).await? else {
            return Ok(0); // no list ever published — nothing of mine exists.
        };
        let dead: std::collections::HashSet<&str> = list.tombstones.iter().map(|t| t.token.as_str()).collect();
        let victims: Vec<invite::InviteEntry> = list
            .entries
            .iter()
            .filter(|e| e.community_id == cid_hex && !dead.contains(e.token.as_str()) && e.epoch.unwrap_or(0) < condemn_below)
            .cloned()
            .collect();
        if victims.is_empty() {
            return Ok(0);
        }
        let mut revoked_tokens: Vec<String> = Vec::with_capacity(victims.len());
        for entry in &victims {
            // One corrupt entry must not wedge the batch (and with it the
            // community's Public/Private reading) forever: tombstone the husk —
            // no revocation can ever be built from it anyway.
            match Keys::parse(&entry.signer_sk) {
                Ok(link_signer) => {
                    let revocation = invite::build_revocation(&link_signer).map_err(|e| e.to_string())?;
                    transport.publish_durable(&revocation, &community.relays).await?;
                }
                Err(_) => crate::log_warn!("[Sever] link {} carries a malformed signer — tombstoning the husk", &entry.token[..entry.token.len().min(8)]),
            }
            list.tombstones.push(invite::InviteTombstone {
                token: entry.token.clone(),
                community_id: cid_hex.clone(),
                extra: Default::default(),
            });
            revoked_tokens.push(entry.token.clone());
        }
        list.entries.retain(|e| !revoked_tokens.contains(&e.token));
        let event = invite::build_invite_list_event_signed(&signer, my_pk, &list).await.map_err(|e| e.to_string())?;
        transport.publish(&event, &community.relays).await?;
        let signers = live_signers_for(&list, &cid_hex, now_ms());
        publish_invite_registry(transport, community, &session, &signers).await?;
        for token in &revoked_tokens {
            let _ = crate::db::community::delete_public_invite(token);
        }
        crate::log_info!("[Sever] revoked {} own public link(s) (containment)", revoked_tokens.len());
        Ok(revoked_tokens.len())
    })
    .await
}

/// The follow-pass severance retry (B4): while a sever mark exists and I still
/// hold live links at-or-below its line, re-attempt. A miss here is load-bearing
/// — a stale registry keeps the community reading Public, which silently skips
/// every future ban's refound — so this runs on every follow until it drains.
pub(crate) async fn retry_pending_severance<T: Transport + ?Sized>(transport: &T, community: &CommunityV2) {
    let cid_hex = crate::simd::hex::bytes_to_hex_32(&community.id().0);
    if sever_marks(&cid_hex).is_empty() {
        return;
    }
    if let Err(e) = sever_own_links(transport, community).await {
        crate::log_warn!("[Sever] pending link severance retry failed (will retry next follow): {e}");
    }
}
...
pub async fn refresh_public_links<T: Transport + ?Sized>(transport: &T, community: &CommunityV2) -> Result<(), String> {
    crate::db::scoped(async move {
        ...
        let now = now_ms();
        let dead: std::collections::HashSet<&str> = list.tombstones.iter().map(|t| t.token.as_str()).collect();
        // A containment condemns every link below its epoch. Refreshing one would
        // RE-ARM the exact door the severing rotation closed (with the new root,
        // no less) — and once re-armed, no retry could tell it from a legitimate
        // link. Skip here; the severance retry owns their burial.
        let condemn_below = sever_marks(&cid_hex).keys().copied().max().unwrap_or(0);
        for entry in &list.entries {
            if entry.community_id != cid_hex || dead.contains(entry.token.as_str()) || entry.token.len() != 2 * super::derive::TOKEN_LEN {
                continue;
            }
            if entry.epoch.unwrap_or(0) < condemn_below {
                continue;
            }
...
pub async fn accept_direct_invite<T: Transport + ?Sized>(transport: &T, wrap: &Event) -> Result<CommunityV2, String> {
    ...
    accept_bundle(transport, &bundle, Some(inviter), true).await
}
...
pub async fn accept_parked_invite<T: Transport + ?Sized>(
    ...
    accept_bundle(transport, &bundle, invited_by, true).await
}
...
pub async fn rescue_stranded_members<T: Transport + ?Sized>(
    ...
    crate::db::scoped(async move {
        ...
        for e in 1..=community.root_epoch.0 {
            ...
            // A rescue extension can fold STANDALONE on a withheld plane, so it
            // must re-state the hop's sever itself: from the fetched siblings, or
            // from the persisted mark when the plane withheld even those.
            let severed_hop = existing
                .iter()
                .any(|c| c.scope == super::rekey::RekeyScope::Root && c.new_epoch == Epoch(e) && c.severed)
                || sever_marks(&cid_hex).contains_key(&e);
...
/// marker, and every adopter — this client included — revokes its own public
...
async fn refound_community_with<T: Transport + ?Sized>(transport: &T, community: &CommunityV2, removed: &[PublicKey], sever: bool) -> Result<CommunityV2, String> {
    crate::db::scoped(async move {
        ...
        // CORD-06 §Authority: a Refounding requires the BAN permission and the rotator
        // must strictly OUTRANK every removed target — the owner is supreme (BAN ⊂
        // owner). Mirrors the receive counterpart (`advance_scope::base_rotator_ok`)
        // and the banlist authority fold: any admin holding BAN may re-found, checked
        // against the folded Roster. Fail-closed — an empty/unauthorized roster leaves
        // only the owner able to re-found.
        {
            ...
            // grant hasn't folded into their own DB (the caller's ban flow folds control
        ...
        let base_chunks =
            super::rekey::build_rekey_chunks(&signer, my_pk, &base_group, super::rekey::RekeyScope::Root, new_epoch, prev_epoch, &prev_commit, &base_blobs, at_secs, my_authority_citation(community, &my_pk).as_ref(), sever)
                .await
                .map_err(|e| e.to_string())?;
        ...
        for ch in &community.channels {
            ...
            let ch_chunks = super::rekey::build_rekey_chunks(&signer, my_pk, &ch_group, super::rekey::RekeyScope::Channel(ch.id), ch_new_epoch, ch.epoch, &ch_prev_commit, &ch_blobs, at_secs, my_authority_citation(community, &my_pk).as_ref(), false)
                .await
                .map_err(|e| e.to_string())?;
        ...
        if sever {
            ...
            // creators' links die when their clients adopt the marked rotation.
            add_sever_mark(&cid_hex, new_epoch.0, at_secs);
            for attempt in 0..3u8 {
                match sever_own_links(transport, &updated).await {
                    Ok(_) => break,
                    Err(e) if attempt == 2 => {
                        crate::log_warn!("v2: post-severing link revocation failed after retries ({e}); the sever mark retries it on the next follow");
                    }
                    Err(_) => continue,
                }
            }
...
pub async fn refound_at_birth<T: Transport + ?Sized>(
    ...
    crate::db::scoped(async move {
        ...
        let base_group = super::derive::base_rekey_group_key(&community.community_root, cid, new_epoch);
        let base_chunks =
            super::rekey::build_rekey_chunks(&signer, my_pk, &base_group, super::rekey::RekeyScope::Root, new_epoch, prev_epoch, &prev_commit, &base_blobs, at_secs, my_authority_citation(community, &my_pk).as_ref(), false)
                .await
                .map_err(|e| e.to_string())?;
...
pub async fn sync_community_list<T: Transport + ?Sized>(transport: &T, bootstrap_relays: &[String]) -> Result<ListSyncOutcome, String> {
    crate::db::scoped(async move {
        ...
        for entry in list.live_entries() {
            ...
            match accept_bundle(transport, &bundle, None, false).await {
                Ok(community) => {
...
/// Merge our OWN just-published Role/Grant into the locally stored roster.
///
/// v2 persists the roster only inside `follow_control`, so a role or grant we
/// just published is invisible to every sync local read (entitlement, capability
/// gates, the next grant) until the next fold. This writes what we are already
/// authorized to have written; the next fold recomputes from the plane and
/// converges. Mirrors the fold's own write, so the stored `roles_at` is left
/// alone — a real edition always outranks this optimistic merge.
...
pub async fn set_role<T: Transport + ?Sized>(transport: &T, community: &CommunityV2, role: &crate::community::roles::Role) -> Result<(), String> {
    ...
    publish_control_edition(transport, community, vsk::ROLE, &role_id, &content).await
}
...
async fn grant_roles_with_roster<T: Transport + ?Sized>(
    ...
    publish_control_edition(transport, community, vsk::GRANT, &eid, &content).await
}
...
pub async fn set_banlist<T: Transport + ?Sized>(transport: &T, community: &CommunityV2, banned: &[String]) -> Result<(), String> {
    crate::db::scoped(async move {
        ...
        publish_control_edition(transport, community, vsk::BANLIST, &eid, &content).await?;
...
// ── Raid containment (CORD-06 Severing Refounding) ──────────────────────────

/// What a containment actually accomplished. Every field is an honest count —
/// a caller (Sentinel, a moderation console) reports these to a human, so a
/// half-done containment must never read as a closed door.
#[derive(Debug, Clone, Default, serde::Serialize)]
pub struct ContainmentReport {
    /// Raiders added to the Banlist.
    pub banned: usize,
    /// Members cut for joining inside the raid window without being convicted.
    pub window_cut: usize,
    /// Root epoch before and after. Equal = the refound did NOT land.
    pub epoch_before: u64,
    pub epoch_after: u64,
    /// True only if the severing refound published.
    pub refound_ok: bool,
    /// My own public links revoked here. Other creators' links die as their
    /// clients adopt the severed rotation.
    pub own_links_revoked: usize,
    /// Creators whose links are still live pending their client's adoption.
    pub foreign_link_creators: Vec<String>,
    /// Non-fatal problems worth showing a human.
    pub warnings: Vec<String>,
}

/// Contain a raid: ban the raiders, cut everyone who walked in through the door
/// during the raid window, then perform a SEVERING Refounding — the rotation
/// that revokes the invite links instead of carrying them into the new epoch.
///
/// This is the raid lane, not a general ban. An ordinary ban (even in a Public
/// community) keeps its shipped behaviour — [`set_members_banned`] decides
/// internally, and a public one deliberately does NOT roll the root. Here the
/// door itself is the vector, so it closes with the rotation.
///
/// `window_start_secs` is the raid's opening edge: unconvicted members who
/// joined at-or-after it are cut from the new epoch (NOT banned — an innocent
/// caught in the window rejoins through the owner's next link). Members already
/// present before the window are never cut, however quiet.
pub async fn contain_raid<T: Transport + ?Sized>(
    transport: &T,
    community_id: &crate::community::CommunityId,
    raiders: &[PublicKey],
    window_start_secs: u64,
    cut_also: &[PublicKey],
) -> Result<ContainmentReport, String> {
    crate::db::scoped(async move {
        let mut report = ContainmentReport::default();
        let community = crate::db::community::load_community_v2(community_id)?
            .ok_or_else(|| "v2 community not found".to_string())?;
        report.epoch_before = community.root_epoch.0;
        report.epoch_after = community.root_epoch.0;
        let owner = community.owner()?;
        let my_pk = me_pk()?;

        // 1. Ban the convicted. Goes through the coalescer: the banlist edition is
        //    a read-modify-write, and a concurrent single ban would otherwise
        //    rebuild from a stale list and erase this one. A FAILED ban (most
        //    plausibly the 500-entry banlist ceiling under a mega-raid) must not
        //    abort the containment: the refound below cuts the raiders
        //    cryptographically whether or not the list could name them.
        if !raiders.is_empty() {
            match set_members_banned(transport, community_id, raiders, true).await {
                Ok(()) => report.banned = raiders.len(),
                Err(e) => report.warnings.push(format!("banlist did not land ({e}) — the refound still cuts every named raider")),
            }
        }

        // Re-read: the ban published a Banlist edition and stripped grants.
        let community = crate::db::community::load_community_v2(community_id)?
            .ok_or_else(|| "v2 community not found".to_string())?;

        // 2. The window cut. A raid arrives THROUGH the link, so everyone who
        //    walked in during it is part of the wave for containment purposes —
        //    conviction only ever names the ones who spoke. Never the owner, never
        //    staff, never anyone whose earliest trace predates the window.
        //
        //    `cut_also` first: Join timestamps are AUTHOR-claimed, so a quiet
        //    raider who backdated their Join slides under any window this fold
        //    can compute. A caller watching arrivals on its own clock (Sentinel's
        //    receive log) names them here and they are cut regardless.
        let mut cut: Vec<PublicKey> = raiders.to_vec();
        for pk in cut_also {
            if !cut.contains(pk) {
                cut.push(*pk);
                report.window_cut += 1;
        ...
        if window_start_secs > 0 {
            let roster = crate::db::community::get_community_roles(
                &crate::simd::hex::bytes_to_hex_32(&community.id().0),
            )
            .unwrap_or_default();
            let owner_hex = owner.to_hex();
            match fetch_guestbook_events(transport, &community, 0).await {
                Ok((events, _, _)) => {
                    // EARLIEST trace per member: a re-announced Join must never make
                    // an established member read as a window joiner.
                    ...
                    for (pk, joined) in first_seen {
                        if joined < window_start_secs || pk == owner || pk == my_pk || cut.contains(&pk) {
                            continue;
                        }
                        // Staff are never cut by a window: a moderator who joined
                        // moments before a raid is still a moderator.
                        if roster.is_staff(&pk.to_hex(), Some(&owner_hex)) {
                            continue;
                        }
                        cut.push(pk);
                        report.window_cut += 1;
                ...
                Err(e) => report
                    .warnings
                    .push(format!("guestbook unreadable — no window cut applied, only convicted raiders are severed ({e})")),
            }
        }

        // 3. The severing refound. Private channels are deliberately NOT rotated
        //    separately: the root roll re-keys every plane, and link-borne raiders
        //    were never entitled to a private channel in the first place.
        //
        //    Under the MODERATION lock, exactly as `run_moderation_batch` holds it
        //    across ITS refound: an ordinary ban racing this rotation would publish
        //    a banlist edition the compaction fetch below has already read past,
        //    and the new epoch would carry it away — a ban silently erased at the
        //    moment bans matter most. Lock order (moderation → follow) matches the
        //    shipped path. Retried: the refound's coverage gate fails closed under
        //    exactly the relay lag a mid-raid flood produces.
        let cid_hex = crate::simd::hex::bytes_to_hex_32(&community.id().0);
        let links_before = crate::db::community::list_public_invites(&cid_hex).map(|l| l.len()).unwrap_or(0);
        let unit = moderation_lock(community_id);
        ...
        match refound {
            Ok(updated) => {
                report.refound_ok = true;
                report.epoch_after = updated.root_epoch.0;
                let links_after = crate::db::community::list_public_invites(&cid_hex).map(|l| l.len()).unwrap_or(0);
                report.own_links_revoked = links_before.saturating_sub(links_after);
                if links_after > 0 {
                    report.warnings.push(format!(
                        "{links_after} of my own link(s) survived the sever — the mark retries them on the next follow"
                    ));
                }
                report.foreign_link_creators = crate::db::community::invite_link_set_creators(&cid_hex)
                    .unwrap_or_default()
                    .into_iter()
                    .filter(|c| *c != my_pk.to_hex())
                    .collect();
            }
            Err(e) => {
                // The banlist landed and silences the named raiders at every honest
                // reader, but the DOOR IS STILL OPEN. Never dress this as success.
                report.warnings.push(format!("SEVERING REFOUND FAILED — the invite door may still be open: {e}"));
            }
        }
        Ok(report)
    })
...
pub async fn edit_community_metadata<T: Transport + ?Sized>(transport: &T, community: &CommunityV2, meta: &control::CommunityMetadata) -> Result<(), String> {
    ...
    publish_control_edition(transport, community, vsk::COMMUNITY_METADATA, &community.id().0, &content).await
}
...
pub async fn edit_channel_metadata<T: Transport + ?Sized>(transport: &T, community: &CommunityV2, channel_id: &ChannelId, meta: &control::ChannelMetadata) -> Result<(), String> {
    ...
    let content = serde_json::to_string(meta).map_err(|e| e.to_string())?;
    publish_control_edition(transport, community, vsk::CHANNEL_METADATA, &channel_id.0, &content).await?;
    ...
    if meta.private {
        if let Some(old) = old_name.filter(|o| *o != meta.name) {
            rename_channel_access_role(transport, community, channel_id, &old, &meta.name).await;
        }
    }
    Ok(())
}
...
/// random key at channel-epoch 1, mint a companion channel-scoped Role that is
/// the channel's access list (CORD-04 §2), deliver the key to the entitled over
/// the rekey plane (CORD-06 §1), then announce the channel (vsk 2, `private`).
/// Epoch 0 is the root generation ("the first privatisation is epoch 1"), so the
/// delivery commits its continuity to `(0, community_root)` — verifiable by every
/// member and bound to THIS community's root. The key ships BEFORE the
/// announcement: an aborted attempt leaves only an unannounced crate (invisible),
/// and a retry mints a fresh id, so there is no same-coordinate double-mint to
/// fork on. Live public links are refreshed; they carry no private key, so this
/// only re-states the public set.
...
/// The companion Role minted alongside a Private channel — the channel's access
...
pub async fn create_private_channel_with_id<T: Transport + ?Sized>(transport: &T, community: &CommunityV2, name: &str, channel_id: ChannelId) -> Result<(), String> {
    crate::db::scoped(async move {
        ...
        // The channel's access list: a companion channel-scoped Role (CORD-04 §2),
        // granted to me so the creator is entitled from the first edition.
        let access_role = channel_access_role(&channel_id, name);
        let access_role_ids = vec![access_role.role_id.clone()];

        // Recipients are the ENTITLED, not the memberlist: CORD-03's private channel
        // is "readable only by granted role-holders". At create that is me (plus the
        // owner, who is always entitled) — everyone else keys up when granted.
        let owner = community.owner()?;
        ...
        let at_secs = now_ms() / 1000;
        let chunks = rekey::build_rekey_chunks(&signer, my_pk, &group, RekeyScope::Channel(channel_id), epoch, Epoch(0), &prev_commit, &blobs, at_secs, my_authority_citation(community, &my_pk).as_ref(), false)
            .await
            .map_err(|e| e.to_string())?;
        for c in &chunks {
            transport.publish_durable(c, &community.relays).await?;
        ...
        publish_control_edition(transport, community, vsk::CHANNEL_METADATA, &channel_id.0, &content).await?;
        // Publish the access list AFTER the channel exists, so a peer folding the
        // Role always resolves the channel it scopes to. A failure here leaves a
        ...
        // The fold is the authority but runs later; without this the creator is not
        // yet entitled to their own channel and the next grant finds no access role.
        merge_local_roster(
            &crate::simd::hex::bytes_to_hex_32(&community.id().0),
            Some(&access_role),
        ...
        // Re-state live links. They carry no private key (CORD-05 §2 — a link's
        // audience holds no Role), so this only refreshes the public set.
...
pub enum VendVerdict {
    /// Every rule passed — adopt the key.
    Accept,
    /// Cannot judge YET: our fold lags the grant it delivers. Park quietly and
    /// re-judge after the next control follow. NOT an anomaly — a lagging fold
    /// is the normal case for a vend that races its own Grant.
    Park(&'static str),
    /// Judged invalid against evidence that cannot become true later. Alarm-worthy.
    Refuse(&'static str),
}

/// Judge a vended Private-Channel key against our OWN folded state.
///
/// The Grant is the authority half and rides the owner-rooted control plane, so
/// it cannot be forged; the vend is only delivery. Acceptance therefore rests
/// entirely on what our own fold proves — a bundle can never introduce a channel
/// our control plane doesn't define, which is what closes the hidden-channel
/// injection class.
///
/// `community` must already be the held (self-certified) community: the caller
/// resolves it by `community_id`, so a bundle naming a community we're not in is
/// never judged here at all.
pub fn judge_channel_key_vend(
    ...
    // (2) The channel must exist in OUR fold, and be private there. The bundle's
    // own claims are ignored: a vend may deliver a key, never define a channel.
    let Some(ch) = community.channel(channel_id) else {
        return VendVerdict::Park("channel not in our fold yet");
    };
    if !ch.private {
        // Never heals: our owner-rooted fold says this channel is public, so a
        // "private key" for it is a spoof, not a lagging view.
        return VendVerdict::Refuse("vend names a channel our fold says is public");
    }

    // (5) Epoch sanity, BOTH directions. Below is superseded by the rotation that
    // produced our copy. Above matters more: the channel head is monotonic, so a
    // wildly-ahead epoch is not merely wrong, it is PERMANENT — every genuine
    // rotation afterwards lands at `head + 1`, is refused as stale, and the
    // channel dies for us with no heal path at all (not a rekey, not a re-grant,
    // not a refound). Rotations advance one epoch at a time, so a lead this large
    // is never a delivery we could place.
    ...
    // (3) OUR fold must show US granted a role scoped to this channel. This is
    // the rule that kills the spoof class: an attacker cannot forge the Grant,
    // so they cannot make us accept a key for a channel we were never granted.
    if !roster.is_entitled(Some(&owner_hex), &me, &chan_hex, &[], &[]) {
        return VendVerdict::Park("our grant for this channel has not folded yet");
    }

    // (4) The vendor must be entitled too — they hold the real key, so a wrong
    // key from them costs isolation, never confidentiality.
...
/// How long an unprovable parked vend is kept. Deliberately long: the fallback
/// heal is the channel's next rotation, which may never come.
const PARKED_VEND_TTL_SECS: u64 = 30 * 24 * 3600;

/// How far above our channel head a vend may claim to be. Generous — a keyless
/// cursor can lag a busy channel by many rotations — but bounded, because the
/// head is monotonic and an over-advance can never be walked back.
const MAX_VEND_EPOCH_LEAD: u64 = 1024;

/// Re-judge every parked key vend for this community and adopt the ones that now
/// pass. Runs after a control follow (the fold moved, so verdicts can change) and
/// on the boot sweep.
///
...
pub fn absorb_parked_channel_keys(community: &CommunityV2, session: &std::sync::Arc<crate::db::Session>) -> Vec<ChannelId> {
    ...
    let now = now_ms() / 1000;
    for p in parked {
        // Several candidates may name one channel (parking is open to any sender,
        // so a stranger can never suppress the entitled vendor's key by holding a
        // slot). Once one is seated the rest are moot.
        ...
        // A vend we were never able to prove is not kept forever: an admin who
        // adds then immediately removes someone leaves a row nothing will ever
        // discharge. Generous by design — the alternative heal (the channel's
        // next rotation) can be arbitrarily far away, so this is hygiene, not a
        // deadline.
        ...
        match judge_channel_key_vend(community, &roster, &channel_id, Epoch(p.epoch), &p.sender) {
            VendVerdict::Accept => {
                ...
                // monotonic guard: it sits at the epoch-0 cursor, and a peer that
                // mints born-private channels at epoch 0 vends that same epoch, so
                // `new > current` would refuse the only key on offer.
                ...
                // The key landed — every other candidate for this channel is moot.
            ...
            VendVerdict::Refuse(why) => {
                ...
                // Only THIS candidate — a sibling may still be the genuine vend.
                let _ = crate::db::community::drop_pending_channel_key(p.id);
            }
            // Quiet by design: the fold simply hasn't caught up.
            VendVerdict::Park(_) => {}
        }
    }
    adopted
...
/// Grant `member` read access to a Private channel (CORD-03 "delivered on
/// grant"): publish a Grant adding the channel's access role, then vend the key
/// as a CORD-05 §6 Direct Invite whose bundle carries exactly the channels they
/// are now entitled to.
///
/// The Grant is the authority half and rides the owner-rooted control plane, so
/// it cannot be forged; the vend is only delivery. A recipient accepts the key
/// solely on the strength of their OWN fold showing this grant — the bundle can
/// never introduce a channel their control plane doesn't define.
pub async fn grant_channel_access<T: Transport + ?Sized>(
    ...
    crate::db::scoped(async move {
        ...
        let chan_hex = crate::simd::hex::bytes_to_hex_32(&channel_id.0);
        let owner_hex = community.owner()?.to_hex();
        // A Grant REPLACES the member's role set, so the union it is built from must
        // be CURRENT: a stale local roster would silently strip every role this
        ...
        // moderator role that happens to share the scope. Pick the permission-less
        // one: conferring read access is exactly what carries no authority.
        let role_id = roster
            .channel_roles(&chan_hex)
            .into_iter()
...
/// Revoke `member`'s read access to a Private channel (CORD-03 "rekeyed on
/// removal"): drop the channel's access role from their Grant, then rotate the
/// channel to its next epoch delivering the fresh key to everyone still
/// entitled (CORD-06). The revoked member keeps whatever history they already
/// read — a rekey protects the future, never the past.
pub async fn revoke_channel_access<T: Transport + ?Sized>(
    ...
    crate::db::scoped(async move {
        ...
        // legitimately entitled member. Refuse rather than mass-evict.
...
/// Rotate one Private channel to its next epoch, delivering the fresh key to
/// everyone entitled EXCEPT `removed` (CORD-06 §1 single-channel rekey).
///
/// `roster` must be the caller's CURRENT view (fetched, not the local cache):
/// the recipient set is built from it, so a cached roster silently drops every
...
async fn rekey_channel_excluding<T: Transport + ?Sized>(
    ...
    crate::db::scoped(async move {
        ...
        // Mint-or-reuse keyed by (channel, next epoch) so a retry after a partial
        // publish re-uses the same key instead of forking the epoch.
        ...
        let at_secs = now_ms() / 1000;
        let chunks = rekey::build_rekey_chunks(&signer, my_pk, &group, RekeyScope::Channel(*channel_id), new_epoch, ch.epoch, &prev_commit, &blobs, at_secs, my_authority_citation(community, &my_pk).as_ref(), false)
            .await
            .map_err(|e| e.to_string())?;
        for c in &chunks {
            transport.publish_durable(c, &community.relays).await?;
            ...
            match read_channel_pins(&rotated, channel_id) {
                Ok(read) if !read.sealed && !read.pins.is_empty() => {
                    ...
                    if let Some(ch2) = rotated.channel(channel_id).cloned() {
                        if let Err(e) = publish_pin_list(transport, &rotated, &ch2, &entries).await {
...
/// The base roots a channel rekey may be addressed under, freshest first: the
/// current root plus the archived priors, capped at [`MAX_ADDRESSING_ROOTS`].
/// CORD-06 D2: a removal-forced channel rekey rides the PRIOR root — so the
/// follower's fetch fan ([`follow_rekeys`]) and the stream-auth registration
/// (`streamauth::register_community`) MUST cover the SAME set. A plane the
/// fetch addresses but auth never registered is invisible on an AUTH-gating
/// relay: the REQ is CLOSED, the rotation crate never arrives, and the channel
/// wedges at its old epoch while the base advances.
...
pub async fn explain_epoch_state<T: Transport + ?Sized>(
    ...
    let reports: Vec<serde_json::Value> = rotations
        ...
        .map(|r| {
            ...
            // CORD-06 §Authority: a Refounding is authorized by BAN in the folded
            // Roster, not owner-identity — report that gate, not just owner-equality.
            ...
            // Is the OWNER a recipient? A non-owner Refounding that drops the owner
            // is a takeover attempt — this tells whether an "owner must be kept"
            // adopt-block would be safe here (it would falsely reject a legitimate
            // rotation that happened to exclude the owner).
            let owner_kept = r.rotator == owner
                || rekey::find_my_blob(&r.blobs, &r.rotator.to_bytes(), &owner.to_bytes(), r.scope, r.new_epoch).is_some();
            // The exact reason follow_rekeys skipped/rejected this rotation, in gate order.
...
pub async fn follow_rekeys<T: Transport + ?Sized>(
    ...
    crate::db::scoped(async move {
        ...
        // CORD-06 §Authority: a Refounding requires the BAN permission in the folded
        // Roster (NOT owner-identity) — any admin holding BAN may perform it, checked
        // against the Roster exactly like a channel rekey checks MANAGE_CHANNELS. The
        // owner is always authorized. (Owner-only here silently wedged every member
        // whose community was refounded by a non-owner admin.)
        ...
        // Concluding MY removal via a base rotation takes more than the bit: the
        // rotator must strictly outrank ME with BAN (CORD-06 §Authority — "the
        // Rotator must strictly outrank every removed target"), so an equal-rank
        // admin can never evict a peer (or the owner) by minting a rotation that
        // skips their blob. Adoption (I hold a blob) only needs `base_rotator_ok`.
        ...
        for _ in 0..MAX_STEPS {
            ...
            // The roots a channel rekey may be addressed under (re-read each pass —
            // a base adopt below changes the head, and its predecessor is already
            // archived). Shared with streamauth so the auth registration covers
            // exactly this fan.
            ...
            // (refresh_subscription recomputes the author-set from the new root).
            {
                ...
                match advance_scope(&batches, RekeyScope::Root, cur.id(), &base_rotator_ok, &base_rotator_outranks_me, &base_admissible, &signer, &my_xonly, next).await {
                    Advance::Adopt { new_key, control_pk, control_root, severed } => {
                        ...
                        // present the containment at the wrong epoch.
                        if severed {
                            severed_hops.push(next.0);
                        }
                        cur.community_root = new_key;
                        cur.root_epoch = next;
        ...
        if !changed {
            // A pending severance whose revoke was interrupted must heal on QUIET
            // passes too — waiting for the next rotation means a one-shot transport
            // blip leaves the community reading Public (and every future ban
            // refound-less) until someone happens to rotate again.
            retry_pending_severance(transport, &cur).await;
        ...
        // Mark every containment hop FIRST: the marks are what condemn links, and
        // both the severance below and the refresh consult them — so the order of
        // those two stops mattering (a condemned link is skipped by the refresh
        // and buried by the severance, in either order, on any pass).
        for hop in &severed_hops {
            add_sever_mark(&cid_hex, *hop, now_ms() / 1000);
        }
        if !severed_hops.is_empty() {
            // Someone ELSE ran a Severing Refounding: my links minted below the
            // severed epoch are the raid's door — bury them instead of carrying
            // them forward. A fresh link minted at-or-after it survives every
            // device's adopt, however late (the epoch is the same on all of them).
            if let Err(e) = sever_own_links(transport, &cur).await {
                crate::log_warn!("[Sever] own-link revocation on adopt failed (retries next follow): {e}");
            }
        }
        // Carry my SURVIVING links across the rotation someone ELSE performed
        ...
        // the stranding the stable-URL refresh exists to prevent. Condemned links
        // are skipped inside (see the guard there). Best-effort and idempotent.
        let _ = refresh_public_links(transport, &cur).await;
        // A pending sever mark (mine or adopted) whose revoke never completed
        // keeps retrying while condemned links remain (B4).
        retry_pending_severance(transport, &cur).await;
...
async fn fetch_rekey_chunks<T: Transport + ?Sized>(
    ...
    for _ in 0..REKEY_MAX_PAGES {
        ...
        // Authenticate AS the rekey plane key: on AUTH-gating relays (Ditto) the
...
async fn advance_scope<S: crate::signer::VectorSigner + ?Sized>(
    ...
    // Parallel to `winners`: whether each delivery's rotation is a containment
...
async fn publish_pin_list<T: Transport + ?Sized>(
    ...
    crate::db::scoped(async move {
        ...
        publish_control_edition(transport, community, vsk::PINS, &eid, &content).await?;
...
pub async fn pin_message<T: Transport + ?Sized>(
    ...
    crate::db::scoped(async move {
        ...
        publish_pin_list(transport, community, &ch, &entries).await
    })
    .await
}
...
pub async fn unpin_message<T: Transport + ?Sized>(
    ...
    let kept: Vec<super::pins::PinEntry> = entries
        ...
        .collect();
    publish_pin_list(transport, community, &ch, &kept).await
}
...
pub(crate) fn spawn_pin_duty(channel_hex: &str, target_rumor_hex: &str, edit: Option<super::stream::OpenedStream>) {
    ...
    crate::db::spawn_bound(async move {
        let transport = crate::community::transport::LiveTransport::with_timeout(std::time::Duration::from_secs(12));
        let _ = run_pin_duty(&transport, &channel_hex, &target, edit).await;
    });
}
...
mod tests {
    ...
    /// Bumps the session generation on the first `publish_durable` — the rekey
    /// crate a private-channel create ships before it writes anything locally.
    ...
    fn addressing_roots_fan_current_plus_archived_bounded_and_deduped() {
        // follow_rekeys' fetch fan AND streamauth's plane registration share
        // this. A channel rekey rides the PRIOR root (CORD-06 D2), so the set
        // MUST include archived roots or an AUTH-gated relay never serves the
        // rotation crate → the channel stalls at its old epoch.
        let (_tmp, _guard, _owner) = init_test_db();
        let cur_root = [9u8; 32];
        let cid = crate::community::CommunityId([1u8; 32]);
        let cid_hex = cid.to_hex();

        // No archives yet → just the current root.
        ...
        // Archive two prior roots (freshest-first ordering is asserted below).
        ...
        // Freshest-archived-first (epoch 1 before epoch 0).
        assert_eq!(roots[1], [2u8; 32], "higher archived epoch is addressed before the lower");

        // A stored root equal to the CURRENT one must not duplicate.
        ...
        // Cap: many archives truncate to MAX_ADDRESSING_ROOTS.
        for e in 3..20u64 {
    ...
    async fn guestbook_store_seeds_syncs_incrementally_and_matches_the_live_fold() {
        ...
        assert!(!sync_guestbook(&bed.relay, &joined).await.unwrap().is_empty(), "the seed folds fresh events");
        ...
        assert!(sync_guestbook(&bed.relay, &joined).await.unwrap().is_empty());
        ...
        bed.swap_to(&member);
        assert!(!sync_guestbook(&bed.relay, &joined).await.unwrap().is_empty(), "the kick lands incrementally");
        assert!(
    ...
    async fn an_old_build_deletion_recovers_from_the_stale_bundle_across_the_split() {
        ...
        let folded = follow_control(&bed.relay, &healed).await;
    ...
    async fn an_unopenable_blob_at_my_locator_never_concludes_removal() {
        ...
        let chunks = rekey::build_rekey_chunks_local(&owner.keys, &base_group, RekeyScope::Root, new_epoch, Epoch(0), &prev_commit, &[garbled, own], 2_000, None, false).unwrap();
        for c in &chunks {
    ...
    async fn a_ban_holding_admin_can_re_found_but_not_evict_a_superior() {
        // CORD-06 §Authority: a Refounding requires BAN, not owner-identity. A
        // non-owner admin granted BAN CAN re-found (and every member follows it —
        // see the receive-side test), but the "strictly outrank every removed
        // target" rule still holds: they can't use it to evict the owner.
        let (bed, owner, member) = TestBed::new();
        bed.swap_to(&owner);
        ...
        bed.swap_to(&member);
        let joined = accept_parked_invite(&bed.relay, &bundle_json, None).await.unwrap();
        // Fold the roster so this member's own DB reflects their BAN grant (the
        // authority check reads the folded Roster, not the bundle).
        let _ = follow_control(&bed.relay, &joined).await;
        let joined = crate::db::community::load_community_v2(joined.id()).unwrap().unwrap();
        // Can't evict the owner (no one outranks the owner).
        ...
        // But CAN re-found removing a plain member they outrank (here, nobody).
    ...
    async fn follow_rekeys_adopts_an_authorized_non_owner_base_rotation() {
        // A BAN-holding ADMIN (not the owner) re-founds, and every member must
    ...
    #[tokio::test]
    async fn adopting_a_severing_rotation_revokes_my_own_links_instead_of_refreshing_them() {
        // The inverse of the sibling test: a SEVERING refound flips the stable-URL
        // contract. A raid arrives through the door, so every creator's client
        // buries its own links on adopt rather than carrying them into the new
        // epoch — which is the only way the OWNER's link dies when a bot admin ran
        // the containment (only the creator holds a link's signer secret).
        let (bed, owner, me) = TestBed::new();
        bed.swap_to(&owner);
        let community = create_community(&bed.relay, "SeverHeal", bed.relays.clone(), None).await.unwrap();
        ...
        assert!(fetch_public_bundle(&bed.relay, &minted.url).await.is_ok(), "my link is live before the raid");

        // The owner runs a SEVERING refound (the raid lane).
        let new_root = [0xD5; 32];
        ...
        let updated = follow_rekeys(&bed.relay, &joined, &crate::db::current_session()).await.unwrap().updated
            .expect("the owner's severing Refounding is adopted");
        assert_eq!(updated.root_epoch, Epoch(1), "I still advance to the new epoch");

        let err = fetch_public_bundle(&bed.relay, &minted.url).await.unwrap_err();
        assert!(err.contains("revoked"), "my link must be a grave, not a refreshed door: {err}");
        assert!(
            crate::db::community::list_public_invites(&crate::simd::hex::bytes_to_hex_32(&updated.id().0)).unwrap().is_empty(),
            "and the local record goes with it",
        );
    }

    #[tokio::test(flavor = "multi_thread")]
    async fn a_severing_adopt_never_deadlocks_the_last_link_creator() {
        // The severance runs INSIDE the follow, which holds `follow_lock` for the
        // whole call. `revoke_public_link`'s last-live-link escalation re-founds —
        // taking that same non-reentrant lock — so routing severance through it
        // would wedge this community's follow forever, for the very client whose
        // revocation empties the registry (typically the owner). The escalation-free
        // path is the fix; this test is its guard.
        let (bed, owner, me) = TestBed::new();
        bed.swap_to(&owner);
        let community = create_community(&bed.relay, "NoDeadlock", bed.relays.clone(), None).await.unwrap();
        let rid = "b2".repeat(32);
        publish_role(&bed.relay, &community, &owner.keys, &admin_role(&rid, Permissions::BAN | Permissions::CREATE_INVITE), 1).await;
        publish_grant(&bed.relay, &community, &owner.keys, &me.keys.public_key(), vec![rid], 1).await;
        let bundle = bundle_of(&community, BundleAudience::Link, Some(owner.keys.public_key()), None, None);
        ...
        let _ = follow_control(&bed.relay, &joined).await;
        let joined = crate::db::community::load_community_v2(joined.id()).unwrap().unwrap();
        // My link is the ONLY one in the registry — revoking it empties the
        // aggregate, which is exactly the escalation's trigger condition.
        let minted = mint_public_link(&bed.relay, &joined, "https://x", None, None).await.unwrap();
        assert!(fetch_public_bundle(&bed.relay, &minted.url).await.is_ok());
        ...
        .await
        .expect("the severing follow must not deadlock on follow_lock")
        .unwrap();
        assert_eq!(followed.updated.expect("adopted").root_epoch, Epoch(1));
    }

    #[tokio::test(flavor = "multi_thread")]
    async fn a_severing_adopt_spares_a_link_minted_after_the_raid() {
        // Severance is bounded by the rotation's own wire stamp, not "now". A
        // second device that adopts late must not eat the fresh link the owner
        // minted to re-open the community after the raid was over.
        let (bed, owner, me) = TestBed::new();
        bed.swap_to(&owner);
        let community = create_community(&bed.relay, "FreshDoor", bed.relays.clone(), None).await.unwrap();
        let rid = "b3".repeat(32);
        publish_role(&bed.relay, &community, &owner.keys, &admin_role(&rid, Permissions::BAN | Permissions::CREATE_INVITE), 1).await;
        publish_grant(&bed.relay, &community, &owner.keys, &me.keys.public_key(), vec![rid], 1).await;
        let bundle = bundle_of(&community, BundleAudience::Link, Some(owner.keys.public_key()), None, None);
        let bundle_json = serde_json::to_string(&bundle).unwrap();
        ...
        let _ = follow_control(&bed.relay, &joined).await;
        let joined = crate::db::community::load_community_v2(joined.id()).unwrap().unwrap();
        let raid_era = mint_public_link(&bed.relay, &joined, "https://old", None, None).await.unwrap();

        let new_root = [0xD7; 32];
        publish_severing_base_rotation(&bed.relay, &joined, &owner.keys, &[owner.keys.public_key(), me.keys.public_key()], &new_root, &joined.community_root).await;
        let updated = follow_rekeys(&bed.relay, &joined, &crate::db::current_session()).await.unwrap().updated.expect("adopted");
        assert!(
            fetch_public_bundle(&bed.relay, &raid_era.url).await.unwrap_err().contains("revoked"),
            "the raid-era link is buried",
        );

        // The raid is over and the owner deliberately re-opens the community. The
        // pending-severance retry runs on every follow, so a door minted after the
        // containment must survive them all — otherwise no one could ever re-open.
        let fresh = mint_public_link(&bed.relay, &updated, "https://new", None, None).await.unwrap();
        let _ = follow_rekeys(&bed.relay, &updated, &crate::db::current_session()).await.unwrap();
        assert!(
            fetch_public_bundle(&bed.relay, &fresh.url).await.is_ok(),
            "a link minted after the containment survives later follow passes",
        );
    }

    #[tokio::test(flavor = "multi_thread")]
    async fn an_unauthorized_severing_rotation_never_touches_my_links() {
        // The sever tag grants no authority of its own: it rides a rotation the
        // adopt gate has already verified. A plain member's forged severing
        // rotation is refused whole, so the tag is never even read.
        let (bed, owner, me) = TestBed::new();
        bed.swap_to(&owner);
        let community = create_community(&bed.relay, "NoForge", bed.relays.clone(), None).await.unwrap();
        let rid = "b4".repeat(32);
        publish_role(&bed.relay, &community, &owner.keys, &admin_role(&rid, Permissions::CREATE_INVITE), 1).await;
        publish_grant(&bed.relay, &community, &owner.keys, &me.keys.public_key(), vec![rid], 1).await;
        let bundle = bundle_of(&community, BundleAudience::Link, Some(owner.keys.public_key()), None, None);
        let bundle_json = serde_json::to_string(&bundle).unwrap();
        bed.swap_to(&me);
        ...
        // A stranger with no BAN mints a severing rotation.
        let stranger = Keys::generate();
        publish_severing_base_rotation(&bed.relay, &joined, &stranger, &[me.keys.public_key(), owner.keys.public_key()], &[0xD8; 32], &joined.community_root).await;
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn a_pending_severance_heals_on_a_quiet_follow_pass() {
        // The retry must run on passes that adopt NOTHING. A one-shot transport
        // blip during the original severance otherwise has no healing trigger
        // until some future rotation happens to be adopted — and until then the
        // community reads Public, which silently strips the refound from every
        // ordinary ban.
        let (bed, owner, me) = TestBed::new();
        bed.swap_to(&owner);
        let community = create_community(&bed.relay, "QuietHeal", bed.relays.clone(), None).await.unwrap();
        let rid = "b5".repeat(32);
        publish_role(&bed.relay, &community, &owner.keys, &admin_role(&rid, Permissions::BAN | Permissions::CREATE_INVITE), 1).await;
        publish_grant(&bed.relay, &community, &owner.keys, &me.keys.public_key(), vec![rid], 1).await;
        let bundle = bundle_of(&community, BundleAudience::Link, Some(owner.keys.public_key()), None, None);
        let bundle_json = serde_json::to_string(&bundle).unwrap();
        ...
        // A containment was adopted but its revoke was interrupted: the mark is
        // down, the epoch-0 link still stands.
        let cid_hex = crate::simd::hex::bytes_to_hex_32(&joined.id().0);
        add_sever_mark(&cid_hex, 1, 5_000);
        assert!(fetch_public_bundle(&bed.relay, &minted.url).await.is_ok());

        // A QUIET follow — no rotation anywhere — must finish the burial.
        let followed = follow_rekeys(&bed.relay, &joined, &crate::db::current_session()).await.unwrap();
        assert!(followed.updated.is_none(), "nothing to adopt");
        assert!(
            fetch_public_bundle(&bed.relay, &minted.url).await.unwrap_err().contains("revoked"),
            "the interrupted severance healed with no rotation to carry it",
        );
    }

    #[tokio::test(flavor = "multi_thread")]
    async fn a_sever_survives_a_later_ordinary_rotation_in_the_same_walk() {
        // Sticky severance: a client catching up walks EVERY missed rotation in
        // one pass. If the containment hop is followed by an ordinary one,
        // last-hop-wins semantics would refresh the links right back open — the
        // door the raid came through, re-armed with the newest root.
        let (bed, owner, me) = TestBed::new();
        bed.swap_to(&owner);
        let community = create_community(&bed.relay, "StickySever", bed.relays.clone(), None).await.unwrap();
        let rid = "b6".repeat(32);
        publish_role(&bed.relay, &community, &owner.keys, &admin_role(&rid, Permissions::BAN | Permissions::CREATE_INVITE), 1).await;
        publish_grant(&bed.relay, &community, &owner.keys, &me.keys.public_key(), vec![rid], 1).await;
        let bundle = bundle_of(&community, BundleAudience::Link, Some(owner.keys.public_key()), None, None);
        let bundle_json = serde_json::to_string(&bundle).unwrap();
        ...
        // Hop 1: the containment. Hop 2: an ordinary rotation on top of it.
        let root1 = [0xE1; 32];
        publish_severing_base_rotation(&bed.relay, &joined, &owner.keys, &[owner.keys.public_key(), me.keys.public_key()], &root1, &joined.community_root).await;
        let mut stepped = joined.clone();
        stepped.community_root = root1;
        stepped.root_epoch = Epoch(1);
        let root2 = [0xE2; 32];
        publish_base_rotation(&bed.relay, &stepped, &owner.keys, &[owner.keys.public_key(), me.keys.public_key()], &root2, &root1).await;

        let updated = follow_rekeys(&bed.relay, &joined, &crate::db::current_session()).await.unwrap().updated.expect("both hops adopt");
        assert_eq!(updated.root_epoch, Epoch(2), "the walk reached the ordinary head");
        assert!(
            fetch_public_bundle(&bed.relay, &minted.url).await.unwrap_err().contains("revoked"),
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn contain_raid_bans_cuts_and_severs_end_to_end() {
        let (bed, owner, _other) = TestBed::new();
        bed.swap_to(&owner);
        let community = create_community(&bed.relay, "Containment", bed.relays.clone(), None).await.unwrap();
        let minted = mint_public_link(&bed.relay, &community, "https://x", None, None).await.unwrap();

        // Three arrivals: an established member (before the window), a convicted
        // raider and a quiet lurker (both inside it).
        let established = Keys::generate();
        let raider = Keys::generate();
        let lurker = Keys::generate();
        let gb = super::super::derive::guestbook_group_key(&community.community_root, community.id(), community.root_epoch);
        for (who, at_ms) in [(&established, 2_000_000u64), (&raider, 9_000_000), (&lurker, 9_500_000)] {
        ...
        let report = contain_raid(&bed.relay, community.id(), &[raider.public_key()], 5_000, &[]).await.unwrap();
        assert_eq!(report.banned, 1, "the convicted raider is banned");
        assert_eq!(report.window_cut, 1, "the lurker who joined inside the window is cut");
        assert!(report.refound_ok, "the severing refound landed: {:?}", report.warnings);
        assert_eq!((report.epoch_before, report.epoch_after), (0, 1), "the root rolled");
        assert_eq!(report.own_links_revoked, 1, "the door closed with it");

        let cid_hex = crate::simd::hex::bytes_to_hex_32(&community.id().0);
        let banlist = crate::db::community::get_community_banlist(&cid_hex).unwrap();
        assert!(banlist.contains(&raider.public_key().to_hex()), "raider on the banlist");
        assert!(!banlist.contains(&lurker.public_key().to_hex()), "the lurker is cut, never banned — they can rejoin through a fresh link");
        assert!(!banlist.contains(&established.public_key().to_hex()));
        assert!(
            fetch_public_bundle(&bed.relay, &minted.url).await.unwrap_err().contains("revoked"),
            "the invite URL the raid used is a grave",
        );

        // The rotation's recipient set is the severance line: the established
        // member rides into the new epoch, the raider and the lurker do not.
        let updated = crate::db::community::load_community_v2(community.id()).unwrap().unwrap();
        assert_eq!(updated.root_epoch, Epoch(1));
        let members = memberlist(&bed.relay, &updated).await.unwrap();
        assert!(members.contains(&established.public_key()), "established member survives");
        assert!(!members.contains(&raider.public_key()), "raider gone");
    }
    ...
    async fn e2e_two_accounts_channels_converse_moderate() {
        ...
        // A private channel is readable only by granted role-holders (CORD-03), so
        // B is added to its access list before the bundle is minted.
        ...
        // ── A creates a SECOND private channel while B is already a member. B is
        // NOT on its access list, so B learns the channel exists (control-follow,
        // keyless) and gets no key: CORD-03's private channel is readable only by
        // granted role-holders, never by every member. B keys up if and when A
        // grants them the channel's access role and vends the key ──
    ...
    async fn a_wide_community_survives_refoundings_and_an_offline_member_converges() {
        ...
        // Private channels are readable only by granted role-holders (CORD-03).
        for id in &priv_ids {
    ...
    async fn an_offline_member_catches_up_across_three_refoundings() {
        ...
        let vault = create_private_channel(&bed.relay, &community, "vault").await.unwrap();
        community = crate::db::community::load_community_v2(community.id()).unwrap().unwrap();
        // The sleeper is on this channel's access list, so the refoundings that
        // follow deliver its key to them (CORD-03).
        ...
        // The sleeper is on vault's access list, but it was created AFTER the last
        // refounding — no rotation followed the grant, so no blob was ever
        // addressed to them. They hold the channel keyless until the grant's own
        // key vend lands (CORD-05 §6), which is what a rekey-only walk cannot do.
    ...
    async fn follow_control_records_a_new_private_channel_keyless_and_unreadable() {
        ...
        // And a bundle minted while keyless never carries the placeholder — a
        // MEMBER audience, so it's the keyless filter proving it (the link
        // filter would drop the channel for the weaker reason).
    ...
    async fn a_link_bundle_never_carries_a_private_channel_key() {
        // A link's audience holds no Role by construction (CORD-05), so a HELD
        // private key must never ride a link bundle — anyone with the URL would
        // get the channel. A member bundle carries it; a link bundle only the
        // public channels.
        let (_tmp, _guard, _owner) = init_test_db();
        let relay = MemoryRelay::new();
        ...
        // A member bundle grants it only to the ENTITLED. An unrelated npub holds
        // no scoped role, so it gets nothing; the creator (granted the companion
        // access role at create) gets the key.
    ...
    async fn a_private_channel_mints_its_access_role_and_entitlement_follows_the_grant() {
        // CORD-03/04: the roles scoped to a channel ARE its access list. Proven
        // against a NON-owner so the owner-is-always-entitled rule can't carry it.
        let (_tmp, _guard, _owner) = init_test_db();
        let relay = MemoryRelay::new();
        ...
        // A stranger holds no scoped role: unentitled, and no key rides their bundle.
        let stranger = Keys::generate().public_key();
        let owner_hex = community.owner().unwrap().to_hex();
        assert!(!roster.is_entitled(Some(&owner_hex), &stranger.to_hex(), &chan_hex, &[], &[]));

        // Granting the access role entitles them; revoking un-entitles them. Both
        // proven through the roster, which is what routes keys.
        ...
        // The access summary a bot reads back: roles, holders, and key state.
    ...
    async fn a_vended_key_parks_until_the_fold_proves_the_grant_then_adopts() {
        // JSKitty's race: the vend can land BEFORE the control fold that proves
        // the grant. It must park quietly (a lagging fold is not an anomaly) and
        // be adopted on the re-judge once the roster catches up.
        let (bed, owner, member) = TestBed::new();
        bed.swap_to(&owner);
        ...
        // Judge as the MEMBER — the owner is always entitled, so only a non-owner
        // can exercise the grant rule at all.
        bed.swap_to(&member);
        let me = member.keys.public_key().to_hex();
        // Their fold has the channel (control-follow records it keyless) but not
        // yet the grant that entitles them.
        ...
        // Ungranted → PARK, never refuse: this is exactly the "not synced enough
        // to judge" case, and it must stay quiet and retryable.
        ...
        // A channel our fold says is PUBLIC never heals — that's a spoof shape.
        ...
        // An unknown channel parks (our fold may simply be behind), never refuses.
        assert!(matches!(
            judge_channel_key_vend(&member_view, &empty, &ChannelId([0x77; 32]), Epoch(1), &owner_hex),
            VendVerdict::Park(_)
        ));

        // Park the vend, then re-judge with a roster that still lacks our grant:
        // it must SURVIVE, not be discarded.
        ...
        // The fold catches up: our grant lands, so the same vend now adopts.
    ...
    async fn a_vend_at_epoch_zero_is_adopted_onto_a_keyless_channel() {
        // Live cross-client finding: a peer that mints born-private channels at
        // epoch 0 vends epoch 0, which collides with our keyless cursor (also 0).
        // The monotonic guard (`new > current`) would refuse the only key we are
        // ever offered, and refuse it SILENTLY. First delivery is not a rotation.
        let (bed, owner, member) = TestBed::new();
        bed.swap_to(&owner);
        ...
        bed.swap_to(&member);
        // The member's view: channel known, keyless, parked at the epoch-0 cursor.
        let mut member_view = held.clone();
        if let Some(c) = member_view.channels.iter_mut().find(|c| c.id.0 == priv_id.0) {
            c.key = None;
            c.epoch = Epoch(0);
        ...
        // Entitle them, then park a vend AT EPOCH 0 (what the peer actually sends).
        let access = crate::community::roles::Role {
    ...
    async fn a_wildly_ahead_vend_epoch_is_refused_not_seated() {
        // The channel head is MONOTONIC, so over-advancing it can never be walked
        // back: every genuine rotation afterwards lands at head+1, reads as stale,
        // and the channel dies for us with no heal path at all. An entitled
        // insider vending a garbage key costs isolation (accepted); one vending a
        // garbage EPOCH would cost the channel permanently, which is not.
        let (bed, owner, member) = TestBed::new();
        bed.swap_to(&owner);
        ...
        // Everything else about this vend is valid — only the epoch is absurd.
        assert!(matches!(
        ...
        // The head is untouched, so the genuine vend still lands afterwards.
        let after = crate::db::community::load_community_v2(community.id()).unwrap().unwrap();
    ...
    async fn a_channel_rename_lands_locally_without_waiting_for_the_fold() {
        // The fold is the authority but runs later, so publishing alone leaves the
        // edit reading back stale — it looks like the rename silently failed.
        let (_tmp, _guard, _owner) = init_test_db();
        let relay = MemoryRelay::new();
    ...
    async fn a_squatted_park_row_cannot_suppress_the_genuine_vend() {
        // Parking is reachable by ANY npub that can gift-wrap us — the bundle
        // self-certifies and its inputs are public for a public community. With a
        // single slot per channel, a stranger could pre-park and the admin's real
        // vend would be a silent no-op, leaving the member keyless with no retry.
        // Candidates + judge-them-all is what closes that.
        let (bed, owner, member) = TestBed::new();
        bed.swap_to(&owner);
        ...
        // A stranger squats FIRST, at a higher epoch than the genuine vend.
        ...
        // The admin's real vend arrives after, at the true epoch.
        crate::db::community::park_channel_key(&cid_hex, &chan_hex, 1, &real_key, &owner_hex).unwrap();
        assert_eq!(
            crate::db::community::get_pending_channel_keys(&cid_hex).unwrap().len(),
            2,
    ...
    async fn revoking_without_a_folded_access_role_refuses_instead_of_evicting_everyone() {
        // With no access role folded, the retained-set filter matches NOBODY, so
        // the rotation would cut off every legitimately entitled member while the
        // Grant it published revoked nothing. Reachable with no attacker: the
        // channel was made on another admin's client and its role hasn't folded.
        let (_tmp, _guard, _owner) = init_test_db();
        let relay = MemoryRelay::new();
        ...
        // Neither the cache nor the plane serves the access role — a withholding
        // relay, or a channel minted on another admin's client. (Wiping only the
        // cache is no longer enough: the revoke re-fetches authority first.)
    ...
    async fn publish_base_rotation(
        ...
        let blobs: Vec<_> = recipients
            ...
            .collect();
        let events = rekey::build_rekey_chunks_local(rotator, &group, RekeyScope::Root, new_epoch, prev_epoch, &prev_commit, &blobs, 2_000, my_authority_citation(community, &rotator.public_key()).as_ref(), false).unwrap();
        for e in &events {
            relay.publish(e, &community.relays).await.unwrap();
        }
    }

    /// A base rotation that carries the containment marker (CORD-06 Severing).
    async fn publish_severing_base_rotation(
        relay: &MemoryRelay,
        community: &CommunityV2,
        rotator: &Keys,
        recipients: &[PublicKey],
        ...
        let blobs: Vec<_> = recipients
            ...
            .map(|r| rekey::build_blob_local(rotator.secret_key(), &rotator.public_key().to_bytes(), r, RekeyScope::Root, new_epoch, new_root).unwrap())
            .collect();
        let events = rekey::build_rekey_chunks_local(rotator, &group, RekeyScope::Root, new_epoch, prev_epoch, &prev_commit, &blobs, 2_000, my_authority_citation(community, &rotator.public_key()).as_ref(), true).unwrap();
        for e in &events {
            relay.publish(e, &community.relays).await.unwrap();
        }
    }
    ...
    async fn follow_rekeys_adopts_an_owner_private_channel_rotation() {
        ...
        let events = rekey::build_rekey_chunks_local(&owner, &group, RekeyScope::Channel(priv_id), Epoch(1), Epoch(0), &prev_commit, &[blob], 2_000, None, false).unwrap();
        for e in &events {
            relay.publish(e, &community.relays).await.unwrap();
        }
    ...
    async fn follow_rekeys_holds_on_an_incomplete_rotation() {
        ...
        let rumor = rekey::build_rekey_rumor(owner.public_key(), RekeyScope::Root, new_epoch, Epoch(0), &prev_commit, &[blob], 1, 2, 2_000, None, false).unwrap();
    ...
    async fn follow_rekeys_finds_a_channel_rekey_under_an_archived_prior_root() {
        ...
        for e in rekey::build_rekey_chunks_local(&owner, &group, RekeyScope::Channel(priv_id), Epoch(2), Epoch(1), &prev_commit, &[blob], 2_000, None, false).unwrap() {
            relay.publish(&e, &community.relays).await.unwrap();
        }
    ...
    async fn follow_rekeys_keyless_cursor_walks_past_an_excluding_rotation_then_adopts() {
        ...
        for e in rekey::build_rekey_chunks_local(&owner, &g1, RekeyScope::Channel(priv_id), Epoch(1), Epoch(0), &pc1, &[b1], 2_000, None, false).unwrap() {
            relay.publish(&e, &community.relays).await.unwrap();
        }
        ...
        for e in rekey::build_rekey_chunks_local(&owner, &g2, RekeyScope::Channel(priv_id), Epoch(2), Epoch(1), &pc2, &[b2], 2_100, None, false).unwrap() {
            relay.publish(&e, &community.relays).await.unwrap();
        }
    ...
    async fn follow_rekeys_honors_an_admin_channel_rotation_but_never_a_strangers() {
        ...
        for e in rekey::build_rekey_chunks_local(&admin, &g2, RekeyScope::Channel(priv_id), Epoch(2), Epoch(1), &pc, &[blob], 2_000, my_authority_citation(&community, &admin.public_key()).as_ref(), false).unwrap() {
            relay.publish(&e, &community.relays).await.unwrap();
        }
        let session = crate::db::current_session();
        ...
        for e in rekey::build_rekey_chunks_local(&rogue, &g3, RekeyScope::Channel(priv_id), Epoch(3), Epoch(2), &pc3, &[rb], 2_100, None, false).unwrap() {
    ...
    async fn a_non_outranking_admins_rotation_never_concludes_my_removal() {
        ...
        for e in rekey::build_rekey_chunks_local(&peer, &g2, RekeyScope::Channel(priv_id), Epoch(2), Epoch(1), &pc, &[pb], 2_000, my_authority_citation(&held, &peer.public_key()).as_ref(), false).unwrap() {
        ...
        for e in rekey::build_rekey_chunks_local(&owner.keys, &g2, RekeyScope::Channel(priv_id), Epoch(2), Epoch(1), &pc, &[ob], 2_100, None, false).unwrap() {
            bed.relay.publish(&e, &held.relays).await.unwrap();
        }
        let follow = follow_rekeys(&bed.relay, &held, &session).await.unwrap();
    ...
    async fn a_rekey_plane_flood_cannot_bury_a_genuine_rotation() {
        ...
        for i in 0..260u64 {
            ...
            let rumor = rekey::build_rekey_rumor(rogue.public_key(), RekeyScope::Root, new_epoch, Epoch(0), &prev_commit, &[blob], 1, 1, 3_000 + i, None, false).unwrap();
    ...
    #[tokio::test]
    async fn a_swap_during_create_private_channel_lands_in_the_creating_account() {
        // create_private_channel publishes the key crate, then the channel
        // edition, then whole-row-saves. A swap in that window used to abandon
        // the whole thing — leaving the key crate published to relays but no
        // local channel to open it, which is the worse of the two outcomes.
        //
        // The operation is pinned to the account that started it, so it now
        // finishes into that account's storage, and the account swapped in gets
        // nothing.
        let (bed, owner, _member) = TestBed::new();
        bed.swap_to(&owner);
        let creator = crate::db::current_session();
        ...
        // The key-crate publish inside create swaps the account mid-flight.
        let swap_relay = SwapMidPublish { inner: MemoryRelay::new() };
        create_private_channel(&swap_relay, &community, "ghost").await.unwrap();

        let mine = crate::db::with_session(creator, async {
            crate::db::community::load_community_v2(community.id()).unwrap().unwrap()
        })
        .await;
        assert_eq!(mine.channels.len(), before + 1, "the channel is minted for the account that asked");
        assert!(mine.channels.iter().any(|c| c.name == "ghost"), "and it is the one requested");
    }
    ...
    async fn an_uncited_admin_rotation_is_not_adopted() {
        ...
        for e in rekey::build_rekey_chunks_local(&admin, &g2, RekeyScope::Channel(priv_id), Epoch(2), Epoch(1), &pc, &[blob], 2_000, None, false).unwrap() {
            relay.publish(&e, &community.relays).await.unwrap();
        }
        ...
        for e in rekey::build_rekey_chunks_local(&admin, &g2, RekeyScope::Channel(priv_id), Epoch(2), Epoch(1), &pc, &[blob2], 2_100, cited.as_ref(), false).unwrap() {
            relay.publish(&e, &community.relays).await.unwrap();
        }
        let out = follow_rekeys(&relay, &community, &crate::db::current_session()).await.unwrap();
    ...
    async fn two_admins_racing_a_channel_rotation_converge_on_one_key() {
        ...
        for (signer, k) in [(&a, &key_a), (&b, &key_b)] {
            ...
            for e in rekey::build_rekey_chunks_local(signer, &group, RekeyScope::Channel(priv_id), Epoch(2), Epoch(1), &pc, &[blob], 2_000, my_authority_citation(&community, &signer.public_key()).as_ref(), false).unwrap() {
                relay.publish(&e, &community.relays).await.unwrap();
            }
        }
    ...
    async fn accept_verifies_a_rotated_plane_whose_metadata_head_is_admin_signed() {
        ...
        let joined = accept_bundle(&bed.relay, &bundle, None, true).await.unwrap();
    ...
    async fn only_an_actual_join_publishes_a_guestbook_join() {
        ...
        accept_bundle(&bed.relay, &bundle, None, true).await.unwrap();
        ...
        accept_bundle(&bed.relay, &bundle, None, false).await.unwrap();
    ...
    async fn accept_refuses_a_rotated_plane_with_no_owner_signed_edition() {
        ...
        bed.swap_to(&member);
        let bundle = bundle_of(&rotated, BundleAudience::Link, None, None, None);
        let err = accept_bundle(&bed.relay, &bundle, None, true).await.unwrap_err();
    ...
    async fn follow_control_persists_the_roster_for_sync_local_reads() {
        ...
        let _ = follow_control(&withholding, &community).await;
        let roster = crate::db::community::get_community_roles(&cid_hex).unwrap();
    ...
    async fn a_fresh_join_sees_pins_and_the_banlist_without_waiting_for_a_follow() {
        ...
        let content = crate::community::v2::pins::serialize_public_pin_list(&[entry]).unwrap();
        let eid = crate::community::v2::derive::pins_locator(community.id(), &general);
        publish_control_edition(&bed.relay, &community, vsk::PINS, &eid, &content).await.unwrap();
    ...
    async fn the_deletion_duty_omits_a_pinned_message() {
        ...
        run_pin_duty(&relay, &ch_hex, &rumor_id, None).await.unwrap();
    ...
    async fn the_edit_duty_refreshes_a_pinned_proof() {
        ...
        run_pin_duty(&relay, &ch_hex, &rumor_id, Some(edit_opened.clone())).await.unwrap();
        let after = read_channel_pins(&community, &general).unwrap();

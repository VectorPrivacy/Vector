...
pub fn resample_audio(path: &Path, target_rate: u32) -> Result<Vec<f32>, Box<dyn std::error::Error + Send + Sync>> {
    ...
    // Open the media source using Symphonia for multi-format support
    ...
    // Use the default format reader to probe the stream
    ...
    // Collect all audio data by decoding packets
    ...
    loop {
        ...
        match decoder.decode(&packet) {
            ...
            Err(SymphoniaError::DecodeError(_)) => {
                // Decode errors are not fatal, just skip this packet
                continue;
            },
            Err(err) => return Err(err.into()),
        }
    ...
    println!("Read {} samples from input file", all_samples.len());

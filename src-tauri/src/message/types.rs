...
/// (bytes, extension, name, size)
type AndroidCachedFile = (Arc<Vec<u8>>, String, String, u64);
...
pub static ANDROID_FILE_CACHE: LazyLock<std::sync::Mutex<HashMap<String, AndroidCachedFile>>> =
    LazyLock::new(|| std::sync::Mutex::new(HashMap::new()));

/// (copy on disk, extension, name, size)
type AndroidPickedFile = (std::path::PathBuf, String, String, u64);
...
pub static ANDROID_PICKED_FILES: LazyLock<std::sync::Mutex<HashMap<String, AndroidPickedFile>>> =
    LazyLock::new(|| std::sync::Mutex::new(HashMap::new()));
